...
(function(global){
    ...
    global.RiderHelper = RiderHelper;
...
(function(global, helper){
    ...
    helper.throttle("resize", "RiderResize");
    ...
    var Rider = function(element, options){
        ...
        for (var i = 0; i < elementPool.length; i++) {
            if (elementPool[i] === element) {
                throw new Error('An instance of Rider with this selector already exists.');
            }
        }
    ...
    Rider.prototype.animationEnd = function (e) {
        if(this.sliding && ( helper.hasClass(e.target, helper.format('{0}--show--1', this.options.itemClass)) || helper.hasClass(e.target, helper.format('{0}--hide--1', this.options.itemClass)) )) {
            this.onEnd();
        }
    };
